...
** Object Structure
   ...
   Whereever possible, we use [[schema][Schema.org]] to organize our collections and items.
   To that end, Every collection is defined as a [[schema:Collection]] and every item in a
   collection is a [[schema:CreativeWork]]. The items of a collection are defined
   with the property [[schema:hasPart]].  This is fundamental organization of a
   collection. This [[./docs/generic_diagram.png][Generic Diagram]], describes that relationship.  Since
   [[schema:Collection]] are themselves [[schema:CreativeWork]] then all properties of
   those apply.  Some of the properties that are indexed and in some cases
   faceted in our DAMS include [[schema:name]], [[schema:about]], [[schema:keywords]],
   [[schema:alternativeHeadline]],[[schema:license]],[[schema:creator]],
